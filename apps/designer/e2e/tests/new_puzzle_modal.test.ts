...
// cargo feature is off, so the New-puzzle modal offers a single "Create" button
// (no "No Solution" button), says simply "Choose a grid size.", and every
// created puzzle carries a fixed solution. The mode marker is
...
test.describe('new-puzzle modal (default with-solution-only build)', () => {
  test('modal offers Create and no No Solution button', async ({ page }) => {
    await installTauriStubs(page, null);
    await gotoApp(page);
    await expect(modalTitle(page)).toBeVisible();

    await expect(
      page.locator('p').filter({ hasText: 'Choose a grid size.' }),
    ).toBeVisible();
    await expect(
  ...
  test('Create button creates a With-Solution puzzle', async ({ page }) => {
    await installTauriStubs(page, null);
    await gotoApp(page);
    await expect(modalTitle(page)).toBeVisible();
