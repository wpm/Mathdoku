...
/// Installs the bundled first-launch sample puzzle (ADR-0007) into `state`.
///
/// A partially-caged 4×4 With-Solution puzzle, loaded by the desktop shell
/// when no recent file can be restored, so the initial screen demonstrates
/// the product — cages with derived targets, live candidate marks, room left
/// to draw — instead of explaining it. The state is clean (`dirty` is false)
/// and unbacked (`path` is `None`): closing immediately never prompts to
/// save, and the first save asks for a location.
///
/// # Errors
/// Never fails in practice — the sample is a fixed design consistent with its
/// own solution. The `Result` exists because the `mathdoku` constructors are
/// fallible.
pub fn install_sample(state: &mut AppState) -> Result<State, Error> {
    type SampleCage = (&'static [(usize, usize)], Operator, u64);

    let square: Vec<Vec<N>> = vec![
        vec![1, 2, 3, 4],
        vec![2, 1, 4, 3],
        vec![3, 4, 1, 2],
        vec![4, 3, 2, 1],
    ];
    let solution = Puzzle::from_latin_square(4, &square)?;

    // Each entry is (cells, operator, target), the target read off the square
    // above. Every operator appears at least once, and the cells (2,3) and
    // (3,3) stay uncovered so the very first interaction can be drawing a cage.
    let cages: [SampleCage; 7] = [
        (&[(0, 0), (1, 0)], Operator::Divide, 2),
        (&[(0, 1), (0, 2)], Operator::Add, 5),
        (&[(0, 3), (1, 3)], Operator::Subtract, 1),
        (&[(1, 1), (1, 2)], Operator::Multiply, 4),
        (&[(2, 0), (3, 0), (3, 1)], Operator::Multiply, 36),
        (&[(2, 1), (2, 2)], Operator::Subtract, 3),
        (&[(3, 2)], Operator::Given, 2),
    ];
    let mut puzzle = Puzzle::new(4)?;
    for (positions, operator, target) in cages {
        let cells: Vec<Cell> = positions.iter().map(|&(r, c)| Cell::new(r, c)).collect();
        let poly = Polyomino::from_cells(&cells)?;
        let t = T::try_from(target)
            .map_err(|_| mathdoku::Error::InfeasibleCage(poly.clone(), target))?;
        let cage = Cage::new(4, poly.clone(), operator, t)?;
        puzzle = puzzle
            .insert_cage(&cage)?
            .ok_or(mathdoku::Error::InfeasibleCage(poly, target))?;
    }
...
mod tests {
  ...
  "puzzle": {
    ...
    // ---- First-launch sample puzzle (ADR-0007) ----

    mod sample {
        use crate::{AppState, install_sample};
        use mathdoku::{Cell, Operator};

        #[test]
        fn sample_is_a_clean_unbacked_4x4_with_solution() {
            let mut state = AppState::default();
            let st = install_sample(&mut state).unwrap();
            assert_eq!(st.puzzle.n(), 4);
            assert!(st.solution.is_some(), "the sample is With-Solution");
            assert!(!state.dirty, "closing the sample must not prompt to save");
            assert!(state.path.is_none(), "the first save must ask for a path");
            assert!(state.active.is_none());
        }

        #[test]
        fn sample_leaves_uncovered_cells_to_draw_on() {
            let mut state = AppState::default();
            let st = install_sample(&mut state).unwrap();
            for cell in [Cell::new(2, 3), Cell::new(3, 3)] {
                assert!(
                    !st.puzzle.cages().any(|cage| cage.contains(cell)),
                    "cell {cell:?} should be free for the user's first cage"
                );
            }
        }

        #[test]
        fn sample_shows_every_operator() {
            let mut state = AppState::default();
            let st = install_sample(&mut state).unwrap();
            for op in [
                Operator::Given,
                ...
                assert!(
                    st.puzzle.cages().any(|c| c.operation().operator == op),
                    "the sample should demonstrate {op:?}"
                );
            }
        }

        #[test]
        fn sample_cage_targets_match_its_solution() {
            let mut state = AppState::default();
            let st = install_sample(&mut state).unwrap();
            let solution = st.solution.as_ref().unwrap();
            for cage in st.puzzle.cages() {
                let vals: Vec<u64> = cage
                    .cells()
                    .iter()
                    .map(|&cell| u64::from(solution.get(cell).unwrap().values()[0]))
                    .collect();
                let target = cage.operation().target;
                let actual: u64 = match cage.operation().operator {
                    Operator::Given => vals[0],
                    Operator::Add => vals.iter().sum(),
                    Operator::Multiply => vals.iter().product(),
                    Operator::Subtract => vals[0].abs_diff(vals[1]),
                    Operator::Divide => vals[0].max(vals[1]) / vals[0].min(vals[1]),
                };
                assert_eq!(
                    actual,
                    target,
                    "cage {:?} target disagrees with the sample solution",
                    cage.polyomino()
                );
            }
        }
        ...
        #[test]
        fn sample_replaces_any_previous_document() {
            let mut state = AppState {
                path: Some("/old/path.mathdoku".to_string()),
                dirty: true,
                active: Some(Cell::new(2, 2)),
                ..AppState::default()
            };
            let _ = install_sample(&mut state).unwrap();
            assert!(state.path.is_none());
            assert!(!state.dirty);
        }
    }
