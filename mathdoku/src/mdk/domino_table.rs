...
use crate::mdk::operator::NonCommutative;
...
impl DominoTable {
    pub fn new(
        ...
        let tuples = (1..=m)
            ...
            .filter(|(i, j)| operation.apply(*i, *j) == target)
            .collect();
...
mod tests {
    ...
    fn subtract_fill_c0_contains_all_values_with_valid_partner() {
        ...
        let memo = DominoTable::new(4, &pair(1, 1, 1, 2), NonCommutative::Subtract, 1).unwrap();
    ...
    fn subtract_fill_invalid_cell_returns_error() {
        let memo = DominoTable::new(4, &pair(1, 1, 1, 2), NonCommutative::Subtract, 1).unwrap();
        assert!(matches!(memo.fill(&Cell::new(2, 1)), Err(MissingCell(_))));
    }
    ...
    fn divide_fill_c0_target2_n4() {
        ...
        // c0 candidates: {2,4}
        let memo = DominoTable::new(4, &pair(1, 1, 1, 2), NonCommutative::Divide, 2).unwrap();
    ...
    fn impossible_target_gives_empty_fills() {
        ...
        let memo = DominoTable::new(4, &pair(1, 1, 1, 2), NonCommutative::Subtract, 9).unwrap();
