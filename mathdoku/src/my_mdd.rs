...
impl MonotonicMDD {
    ...
    fn new(n: u32, constraint: MonotonicConstraint) -> Self {
        let mut mmd = Self {
            n,
            constraint,
            edges: HashMap::new(),
...
mod tests {
    ...
    impl MonotonicMDD {
        /// Enumerate all value tuples accepted by this MDD.
        fn tuples(&self) -> Vec<Tuple> {
            let root = Node {
                depth: 0,
                value: self.constraint.unit(),
            };
            let mut result = Vec::new();
            self.tuples_from(root, &mut Vec::new(), &mut result);
            result
        }

        fn tuples_from(&self, head: Node, path: &mut Tuple, result: &mut Vec<Tuple>) {
            match self.edges.get(&head) {
                None => {
                    if head.value == self.constraint.target()
                        && head.depth == self.constraint.arity()
                    {
                ...
                Some(edges) => {
                    for &(label, tail) in edges {
                        path.push(label);
                        self.tuples_from(tail, path, result);
                        let _ = path.pop();
                    }
                }
            }
